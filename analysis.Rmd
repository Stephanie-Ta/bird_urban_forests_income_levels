...
```{r message = FALSE, warning = FALSE}
# Get main stats data and add info to it
...
# save cleaned stats data
write_csv(stats_data, paste0(CLEANED_DATA_FOLDER, "stats_data_cleaned.csv"))

# get economic level data per dissemination area for later
economic_level_per_da <- stats_data |>
  distinct(dissemination_area, economic_level)
```

```{r message = FALSE, warning = FALSE}
# Previous code to clean the plant and shrub data:
...
# Get shrub and plant data and add info to it
plant_shrub_data_clean <- read_delim("data/raw/plant_shrub_data.csv", delim=';') |>
  clean_names() |>  # clean column names
  fill(house, .direction = "down") |>  # fill in missing house values
  mutate(species = str_to_sentence(species),  # standardize capitalization of species names to title case
         type = str_to_lower(type),  # standardize capitalization of vegetation type to lowercase
         native = case_when(native == "native" ~ TRUE, # native/non-native to boolean
                            native == "non" ~ FALSE),
         fruiting = case_when(fruiting == "y" ~ TRUE, # fruiting to boolean
                              fruiting == "n" ~ FALSE),
         flowering = case_when(flowering == "y" ~ TRUE, # flowering to boolean
                               flowering == "n" ~ FALSE),
         grain = case_when(grain == "y" ~ TRUE, # grain to boolean
                           grain == "n" ~ FALSE),
         dissemination_area = as.factor(str_sub(house, 1, 2)),  # add dissemination area
         economic_group_code = as.factor(str_sub(dissemination_area, 1, 1)),  # add economic level group code
         economic_group_code = fct_relevel(economic_group_code, c("P", "M", "R")),  # specify order of economic group levels
         economic_group = as.factor(case_when(economic_group_code == "P" ~ "Low",  # add economic group level name
                                    economic_group_code == "M" ~ "Medium",
                                    economic_group_code == "R" ~ "High")),
         economic_group = fct_relevel(economic_group, c("Low", "Medium", "High"))) |>  # specify order of economic group levels
  left_join(economic_level_per_da, by=join_by(dissemination_area))  # add numeric economic level
```
...
tree_counts_per_da <- subset_tree_data |>
  group_by(dissemination_area, economic_level, name, native_or_non_native, type) |>
  summarize(number = n()) |>
  ungroup() |>
  arrange(economic_level, desc(number)) |>
  mutate(da_el = str_c(dissemination_area, economic_level, sep = ", economic level: "),
         da_el = fct_reorder(da_el, economic_level),
         economic_group_code = as.factor(str_sub(dissemination_area, 1, 1)),
         economic_group_code = fct_relevel(economic_group_code, c("P", "M", "R")),
         economic_group = as.factor(case_when(economic_group_code == "P" ~ "Low",
                                    economic_group_code == "M" ~ "Medium",
                                    economic_group_code == "R" ~ "High")),
         economic_group = fct_relevel(economic_group, c("Low", "Medium", "High")))

tree_top_5_species_counts_per_da <- tree_counts_per_da |>
  group_by(economic_level, .drop = FALSE) |>
  slice(1:5) 
```
...
tree_counts_per_economic_group <- tree_counts_per_da |>
  group_by(economic_group, name) |>
  summarise(number = sum(number))

# # count trees per species per level
# tree_counts <- tree_data |>
#   count(economic_level, name, name = "n_trees")
# 
# # total houses per level
# house_totals <- tree_data |>
#   distinct(house, economic_level) |>
#   count(economic_level, name = "n_houses")
# 
# # normalize
# tree_norm <- tree_counts |>
#   left_join(house_totals, by = "economic_level") |>
#   mutate(trees_per_house = n_trees / n_houses)

ggplot(tree_counts_per_economic_group,
       aes(x = economic_group, y = reorder(name, desc(name)), fill = number)) +
...
plant_shrub_counts_per_da <- plant_shrub_data_clean |>
  group_by(dissemination_area, economic_level, species, native, type) |>
  summarize(number = n()) |>
  ungroup() |>
  arrange(economic_level, desc(number)) |>
  mutate(da_el = str_c(dissemination_area, economic_level, sep = ", economic level: "),
...
ggplot(plant_shrub_top_5_species_counts_per_da,
       aes(x = number, y = fct_reorder(species, -number), fill = type)) +
...
plant_shrub_per_economic_group <- plant_shrub_counts_per_da |>
  group_by(economic_group, type, species) |>
  summarise(number = sum(number))

plant_per_economic_group <- plant_shrub_per_economic_group |>
  filter(type == "plant")

ggplot(plant_per_economic_group,
       aes(x = economic_group, y = reorder(species, desc(species)), fill = number)) +
  geom_tile(colour="black") +
  scale_fill_viridis_c(direction = -1) +
  theme(panel.background = element_blank()) +
  labs(x = "Economic Level",
...
shrub_per_economic_group <- plant_shrub_per_economic_group |>
  filter(type == "shrub")

ggplot(shrub_per_economic_group,
       aes(x = economic_group, y = reorder(species, desc(species)), fill = number)) +
  geom_tile(colour="black") +
  scale_fill_viridis_c(direction = -1) +
  theme(panel.background = element_blank()) +
...
### Poisson Model
We also tried a Poisson Model and a Poison Mixed Effects Model since bird species richness is a count variable.

A Poisson model assumes that there is equidispersion, in which the mean of the response (in this case, bird diversity) is equal to the variation of the response. Overdispersion is when the variation of the response is greater than the mean of the response. In the case of overdispersion, we would use a negative binomial model.

First, we fit a Poisson model *without mixed effects* to check if there is significant dispersion in the data, since `AER::dispersiontest()` does not work on Poisson models with mixed effects.
```{r}
poisson_model <- glm(bird_diversity ~ overall_garden_diversity +
                       canopy_cover_garden +
                       native_percent_total +
                       economic_level, family = poisson, data = stats_data)

dispersiontest(poisson_model)
```
From the dispersion test above, we can see that there is not a significant amount of overdispersion (p-value = 0.107).

Then, we went ahead and fit a Poisson model with mixed effects to take into account that our data points are **not** independent of each other since samples were taken per dissemination area.

```{r, warning=FALSE, message=FALSE}
poisson_me_model <- glmer(bird_diversity ~ overall_garden_diversity +
                            canopy_cover_garden +
                            native_percent_total +
                            economic_level +
                            (1 | dissemination_area),
...
summary(poisson_me_model)$coefficients |>
   as.data.frame() |>
  mutate(adjusted_p_value = p.adjust(`Pr(>|z|)`, 'fdr')) |>
  kable()
```